...
bool ModelMeshPipeline::addObjectsToBeRendered(const std::vector<ModelMeshRenderable *> & additionalObjectsToBeRendered, const bool useAltGraphicsQueue) {
    ...
    if (this->debugPipeline != nullptr) {
        ...
        for (auto r : additionalObjectsToBeRendered) {
            ...
            if (this->showNormals) {
                auto normalsGeom = Helper::getNormalsFromMeshRenderables<ModelMeshRenderable>(r);
                auto normalsMeshRenderable = std::make_unique<VertexMeshRenderable>(this->debugPipeline->getName() + "-bbox" + std::to_string(c), normalsGeom);
